...
)
{
	...
	// Apply knapsack approach
	for (size_t i = 1; i <= foods.size(); i++ ) {
		for (size_t j = 1; j <= total_calories; j++){
			if (i == 0 || j == 0) {
				T[i][j] = 0;
			}
			else if (j - foods[i-1]->calorie() < 0){
				T[i][j] = T[i-1][j];

			} else {
				// T[i,j] = max(T[i-1,j], T[i-1,j-X]+V)
				T[i][j] = std::max(T[i-1][j], (T[i-1][j-foods[i-1]->calorie()] + foods[i-1]->weight()));
			}
		}
	}

    size_t i = foods.size();
    size_t c = total_calories;

    while (i != 0 && c != 0) {
        if (T[i][c] != T[i - 1][c]) {
            best->push_back(foods[i - 1]);
            c = c - foods[i - 1]->calorie();
        }
        i--;
    }
